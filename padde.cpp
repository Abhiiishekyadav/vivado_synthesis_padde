...
void FullyConnectedLayer(const int8_t A[], const Vec_t B[], Vec_t C[], const int8_t bias[], const int8_t scale, int K, int N, int M, int D, int W, bool padde){
		...
		#pragma HLS LOOP_TRIPCOUNT min = 1 max = 1
			...
			#pragma HLS PIPELINE II=1
				...
				for (int i = 0; i < W; ++i) {
					...
					if (padde)
						C[n * D * (M / W) + nd * (M / W) + m][i] = static_cast<int8_t>(pade(static_cast<float>(tmp[i])));
					else
						//C[n * D * (M / W) + nd * (M / W) + m][i] = (tmp[i] < 0) ? 0 : tmp[i];
						C[n * D * (M / W) + nd * (M / W) + m][i] = tmp[i];
						 //for (int i = 0; i < W; ++i) {
						    //float pade_val = pade((float)tmp[i] + (float)bias[i]);
						    //C[n * D * (M / W) + nd * (M / W) + m][i] = (int8_t)(pade_val * 127);
						//}
				}
			}
		}
